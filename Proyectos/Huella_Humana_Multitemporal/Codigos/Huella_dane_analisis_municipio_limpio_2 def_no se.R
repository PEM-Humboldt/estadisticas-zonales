...
df_cor_text <- df_corA %>% 
  filter(Año==2022 & Actividades == "VATarea")

#install.packages("ggpmisc")
library(ggpmisc)

df_plot %>% 
  filter(Año==2022 & Actividades == "VATarea" & variable %in% c("mean")) %>% #c("mean", "sd")
  ggplot(aes(x = VA_Actividades + 0.000001, y = valor_iheh)) +
  ...
  geom_text(
    data = filter (df_cor_text, variable == "mean"),
    aes(
      x = Inf, y = Inf,
      label = paste0(
  ...
  labs(
    title = "Relación entre VA (log) e IHEH con correlaciones y \nfórmula lineal",
    x = "VA (log10)",
    y = "Valor IHEH"
  )
  ...
  mutate(
    ...
    # relacion de cambio
        ...
        categoria_cambio = case_when(
          ...
          Pvt_18_22 > 0 & Pmean_18_22 > 0 & D < 1 ~ "Desacoplamiento relativo",
          Pvt_18_22 > 0 & Pmean_18_22 > 0 & D >= 1 ~ "Crecimiento con alta presión",
          
          # Contracción general
          Pvt_18_22 < 0 & Pmean_18_22 < 0 ~ "Declive económico y ambiental",
