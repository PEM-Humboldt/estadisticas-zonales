...
zonal(as.numeric(raster_interes), r_aoi, fun = "sd", na.rm=T)


z_mean   <- zonalTabla("mean")
z_sd     <- zonal(as.numeric(raster_interes), r_aoi, fun = "sd", na.rm=T)%>%
  tidyr::pivot_longer(
    cols = c(`H2015`),   # columnas de años
    names_to = "Año",
    values_to = "sd"
...
z_median <- zonal(as.numeric(raster_interes), r_aoi, fun = "median", na.rm=T) %>%
  tidyr::pivot_longer(
    cols = c(`H2015`),   # columnas de años
    names_to = "Año",
    values_to = "median"
...
list_deptos [[c(1088,1089)]] #quitar si dpto
quitar <- c(1088,1089) # si dpto , no aplica para biomas, municipios  c(1088,1089) 
...
write_excel_csv2(Stat_values, paste0(dir_Resultados, "/IHEH_stats_2015", Unidad_analsis,".csv"))
write_excel_csv2(Stat_reclass, paste0(dir_Resultados, "/IHEH_clases_2015",Unidad_analsis,".csv"))
